...
procedure TAccep_HRD.CheckBox2Click(Sender: TObject);
begin
     if CheckBox2.Checked = true then
        begin
            MyQuery4.Close;
            MyQuery4.SQL.Text := 'select * from hrd order by h_id desc';
            MyQuery4.Open;
        end;
...
begin
  buttonSelected := MessageDlg('คุณต้องการลบคำร้องของ   '+cxDBLabel2.caption,mtError, mbOKCancel, 0);
...
var
  buttonSelected : Integer;
begin
  buttonSelected := MessageDlg('คุณต้องการเพิ่มคำร้องของ   '+cxDBLabel2.caption+'  ลงในระบบ',mtError, mbOKCancel, 0);
  if buttonSelected = mrOK     then
      begin
      MyQuery3.Close;
      MyQuery3.open;
...
procedure TAccep_HRD.FormClose(Sender: TObject; var Action: TCloseAction);
begin
Release;
end;
...
procedure TAccep_HRD.Image1Click(Sender: TObject);
begin
Close;
end;
