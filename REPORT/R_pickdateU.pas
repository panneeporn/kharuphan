...
uses
  ...
  JvCheckedMaskEdit, JvDatePickerEdit, dxGDIPlusClasses, Vcl.ExtCtrls, frxClass,
  frxDBSet;
...
uses I_ReportU, I_ReportServiceU, ThaiDate, Change_DrugU;
...
begin
     ...
     if I_Report.cxdblabel1.Caption = '1' then
        begin
            ...
            if  Change_Drug.RadioButton1.Checked = true then
                begin
                        ...
                        I_Report.process.sql.Text := ' SELECT op.an,op.hn,pa.cid,op.vstdate,CONCAT(pa.pname,pa.fname,'+'" '+' "'+',pa.lname)as fullname,pa.informaddr,pa.hometel,drug.name,drug.strength,drug.dosageform FROM opitemrece op '+
                                                    ...
                                                    //'limit 1000';
                                                    ' where drug.icode = '+'"'+Change_Drug.label2.caption+'"'+' and op.vstdate BETWEEN '+'"'+formatdatetime('yyyy-mm-dd',R_pickdate.JvDatePickerEdit1.Date)+'"'+' and '+'"'+ formatdatetime('yyyy-mm-dd',R_pickdate.JvDatePickerEdit2.Date)+'"'+' order by op.vstdate ';


                       //ShowMessage(I_Report.process.sql.Text);
                       I_Report.process.open;
                      //  ds1 := I_Report.frxReport_Phamacy1.FindObject('ds1') as TfrxMemoView;
                     //   ds2 := I_Report.frxReport_Phamacy1.FindObject('ds2') as TfrxMemoView;
                      //  ds1.Text := StringReplace(ds1.caption, '%my_str%', 'new string', [rfReplaceAll]);
                     //   ds2.Text := StringReplace(ds2.caption, '%my_str%', 'new string', [rfReplaceAll]);
                       I_Report.frxReport_Phamacy1.PrepareReport;
                       I_Report.frxReport_Phamacy1.Print;

                end;
            if  Change_Drug.RadioButton2.Checked = true then
                begin
                        I_Report.process8.close;
                        I_Report.process8.sql.Text :=' select  op.an,op.hn,p.cid,op.vstdate,CONCAT(p.pname,p.fname,'+'"'+'  '+'"'+',p.lname)as fullname,p.informaddr,p.hometel,non.`name`,op.qty from opitemrece op  '+
                                                     ' LEFT JOIN patient p on op.hn = p.hn '+
                                                     ' LEFT JOIN nondrugitems non on op.icode = non.icode  '+
                                                     ' LEFT JOIN ovst ov on op.vn = ov.vn '+
                                                     ' where non.icode = '+'"'+Change_Drug.label2.caption+'"'+' and op.vstdate BETWEEN '+'"'+formatdatetime('yyyy-mm-dd',R_pickdate.JvDatePickerEdit1.Date)+'"'+' and '+'"'+ formatdatetime('yyyy-mm-dd',R_pickdate.JvDatePickerEdit2.Date)+'"'+' order by op.vstdate ';
                       I_Report.process8.open;
                       I_Report.frxReport_Phamacy2.PrepareReport;
                       I_Report.frxReport_Phamacy2.Print;
                end;
...
procedure TR_pickdate.FormShow(Sender: TObject);
begin
    ds1.Caption := FormatThaiDate('dd mmmm eeee',JvDatePickerEdit1.Date);
    ds2.Caption := FormatThaiDate('dd mmmm eeee',JvDatePickerEdit2.Date);
end;

procedure TR_pickdate.JvDatePickerEdit1Change(Sender: TObject);
begin
ds1.Caption := FormatThaiDate('dd mmmm eeee',JvDatePickerEdit1.Date);
end;

procedure TR_pickdate.JvDatePickerEdit2Change(Sender: TObject);
begin
ds2.Caption := FormatThaiDate('dd mmmm eeee',JvDatePickerEdit2.Date);
end;
