...
begin
   ...
   if MyQuery4.Locate('sum_an',VarArrayOf([cxTextEdit1.Text]),[]) then
      begin
            ...
            ShowMessage('มีAN  นี้อยู่ในระบบแล้ว');
      end
    else
      begin
  ...
  begin
    ...
    MyQuery4.Open;
       ...
       if MyQuery4.Locate('sum_an',VarArrayOf([DBtext9.Caption]),[]) then
         begin
            ShowMessage('มี AN นี้อยู่ในระบบแล้ว');
         end
       else
          begin
              MyQuery4.close;
       ...
       if (strtoint(Ilabel1)) > (strtoint(regis_dis)) then
            ...
            ShowMessage('การบันทึกเวชระเบียนนี้มีความล่าช้า  โดยมีระยะเวลาเกินกำหนด '+datepama +'วัน');
            MyQuery8.Close;
            MyQuery8.Open;
            MyQuery8.Append;
            ...
            MyQuery8.FieldByName('detail').AsString := 'การบันทึกเวชระเบียนลงระบบล่าช้า';
...
if (DBText9.Caption = '-')  or (DBText9.Caption = 'null')   or (DBText9.Caption = ' ')  or (DBText9.Caption = '')   then
  begin
   ShowMessage('ตรวจสอบความถูกต้องของ AN');
  end;
