...
begin
         ...
         FullMemo := Tachnician.DBMemo1.Text + #13#10 +DateToStr(date2)+'::'+
            'ไม่สมารถซ่อมบำรุงได้';
         ShowMessage(Tachnician.DBMemo1.Text);
         ShowMessage(FullMemo);

         MyQuery1.Close;
         MyQuery1.SQL.Text := 'update main_t set text ='+'"'+FullMemo+'"'+' ,status = '+'"'+'N'+'" '+', acc_technician = '+'"'+'Y'+'" '+' , date_acc_technician = '+'"'+formatdatetime('yyyy-mm-dd',Date2)+'"'+' where id = '+'"'+Tachnician.DBText6.Caption+'"';
         MyQuery1.Execute;
