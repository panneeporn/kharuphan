...
uses
  ...
  MemDS, DBAccess, MyAccess, cxLookupEdit, cxDBLookupEdit, cxDBLookupComboBox;
...
begin
     ...
     MyQuery3.FieldByName('assign').AsString :=  cxLookupComboBox1.Text;
     ...
     Date2 := MyQuery2.FieldByName('now()').AsDateTime;
             ...
             FullMemo := Tachnician.DBMemo1.Text + #13#10 +DateToStr(date2)+'::'+'ช่างส่งต่อพัสดุพิจรณา   ';
             ShowMessage(Tachnician.DBMemo1.Text);
             ShowMessage(FullMemo);

             MyQuery1.Close;
             MyQuery1.SQL.Text := 'update main_t set text ='+'"'+FullMemo+'"'+',status_s = '+'"'+'2'+'"'+',status = '+'"'+'B'+'"'+', time_tech1 ='+'"'+inttostr(0)+'"'+', acc_technician = '+'"'+'P'+'" '+' , date_acc_technician = '+'"'+formatdatetime('yyyy-mm-dd',Date2)+'"'+' where id = '+'"'+Tachnician.DBText6.Caption+'"';
             MyQuery1.Execute;
             Tachnician.Showgrid.Refresh;

    ShowMessage('บันทึกข้อมูลเสร็จเรียบร้อย');
    Stat.close;
    SendSubpile.Close;
